...
class AIspeakerAudioSender(ESPTCPAudioSend):
    ...
    def start(self):
        try:
            ...
            if signal == self.SIGNAL_MAC:
                ...
                result=self.lam.msg(text_from_user[1])
...
class AIspeakerAudioReciver(ESP32TCPAudioRecv):
    def __init__(self, client_socket, client_address, work_list, lock):
        super().__init__(client_socket, client_address)
        self.work_list = work_list     
        self.lock = lock
    ...
    def start(self):
        ...
        try:
            ...
            # 위스퍼 문자열생성
            result = self.model.transcribe("3030F9694F90.wav")
            print(result["text"])
